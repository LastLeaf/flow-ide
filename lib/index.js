  ...
  provideLinter(): Object {
    const linter = {
      ...
      lint: async (textEditor) => {
        ...
        let result
        try {
          result = await exec(executable, ['status', '--json'], { cwd: fileDirectory, ignoreExitCode: true, throwOnStdErr: false })
        } catch (error) {
  ...
  provideAutocomplete(): Object {
    const provider = {
      ...
      getSuggestions: async ({editor, bufferPosition, prefix, activatedManually}) => {
        ...
        let result
        try {
          result = await exec(await this.getExecutablePath(fileDirectory), ['autocomplete', '--json', filePath], { cwd: fileDirectory, stdin: fileContents, ignoreExitCode: true, throwOnStdErr: false })
        } catch (_) {
